...
def compare_prices(api_files):
    ...
    for api_file in api_files:
        with open(api_file, 'r') as file:
            api_prices[api_file] = {key: float(value) for key, value in (line.strip().strip(',').split(':') for line in file)}
...
if __name__ == "__main__":
    api_files = ["coinmarketcap.txt", "coingecko.txt", 'coinmarketcap.txt']  
